    ...
    for row in rows:
        if(str(zipcode) in row[9]):
            if("FOREIGN" in row[3]):
                ...
                obj = {}
                obj["jurisdiction"] = str(jurisdiction)
                obj["zipcode"]=str(zipcode)
                json_values.append(obj)
                value =  (str(jurisdiction), str(zipcode))
                values.append(value)
    
    zip_dict[str(zipcode)] = json_values
        
#for i in alljurisdictions:
#    alljurisdictionsF[i]=alljurisdictionsF.get(i,0)+1
for i in values:
    valuesF[i]=valuesF.get(i,0)+1
print valuesF
#print alljurisdictionsF
#print(s)
#basePath = '/Users/jiazhang/Documents/GitHub/enigmaCorporations/'
#output_file = open(basePath+'NewYorkCity_byzip.json', 'w')
